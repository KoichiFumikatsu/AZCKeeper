...
/// <summary>Clase de fallo de red. Determina cuánto conviene esperar antes de reintentar.</summary>
...
/// escalaba hasta 30 minutos. El problema: metía en ese mismo camino los fallos de DNS
/// de la sede, que duran segundos. Un parpadeo de DNS costaba media hora sin handshake —
/// y por tanto sin política ni chequeo de update.
///
/// Un ban no rompe la resolución DNS: si el host no resuelve, esperar 30 minutos no aporta
/// nada. De ahí la separación de topes por clase.
/// </summary>
public sealed class NetworkBackoffPolicy
{
    ...
    public NetworkBackoffPolicy(Func<DateTime> nowUtc, Func<double>? rng)
    {
        ...
        if (rng == null)
        {
            ...
            _rng = () => { lock (random) { return random.NextDouble(); } };
        }
        else
        {
            _rng = rng;
    ...
    /// <summary>Clasifica el resultado de un intento de red. <paramref name="ex"/> es null si hubo respuesta HTTP.</summary>
    ...
    public NetworkFailureKind Register(int statusCode, Exception? ex)
    {
        ...
        lock (_lock)
        {
            ...
            // Cambiar de clase arranca una escalada nueva: los intentos fallidos por DNS no
            // son evidencia de que el servidor nos esté frenando, ni al revés.
            if (kind != _lastKind)
            {
                _consecutiveFailures = 0;
            }
            ...
            {
                // Ya hay backoff activo de esta MISMA clase y este fallo llegó dentro de esa
                // ventana. Solo puede ser un straggler concurrente de la misma caída:
                ...
                // entra aquí, porque el envío no abre socket mientras hay backoff. Contarlo
                // inflaría el contador (una caída se veía como 3) y escalaría el backoff de
                // más. Se ignora sin re-incrementar ni re-extender.
                return kind;
            }
