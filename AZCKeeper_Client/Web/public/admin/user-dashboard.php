...
// Focus Score 0-100 from keeper_focus_daily
$focusDaily = [];
$userFocusScore = 0;
$userFocusBreakdown = null;
try {
    $focusDaily = \Keeper\Repos\ProductivityRepo::getDailyMetrics($pdo, $userId, $dateFrom, $dateTo);
    if (!empty($focusDaily)) {
        $fScores = array_column($focusDaily, 'focus_score');
        $userFocusScore = (int)round(array_sum($fScores) / count($fScores));
        $userFocusBreakdown = [
            'productivity' => (int)round(array_sum(array_column($focusDaily, 'productivity_pct')) / count($focusDaily)),
            'constancy'    => (int)round(array_sum(array_column($focusDaily, 'constancy_pct')) / count($focusDaily)),
            'deep_work'    => (int)round(array_sum(array_column($focusDaily, 'deep_work_seconds')) / count($focusDaily)),
            'switches'     => (int)round(array_sum(array_column($focusDaily, 'context_switches')) / count($focusDaily)),
            'punctuality'  => (int)round(array_sum(array_column($focusDaily, 'punctuality_minutes')) / count($focusDaily)),
        ];
    }
} catch (\Throwable $e) {}

// Dual-job alerts for this user
$userDualAlerts = [];
try {
    $userDualAlerts = \Keeper\Repos\ProductivityRepo::getUserAlerts($pdo, $userId, 5);
} catch (\Throwable $e) {}
...
function focusBadge(float $score): string {
    if ($score >= 75) return '<span class="inline-flex items-center text-xs font-semibold text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-full">Excelente</span>';
    if ($score >= 50) return '<span class="inline-flex items-center text-xs font-semibold text-corp-800 bg-corp-50 px-2.5 py-1 rounded-full">Bueno</span>';
    if ($score >= 30) return '<span class="inline-flex items-center text-xs font-semibold text-amber-700 bg-amber-50 px-2.5 py-1 rounded-full">Regular</span>';
...
function focusColor(float $score): string {
    ...
    if ($score >= 50) return 'text-corp-800';
    if ($score >= 30) return 'text-amber-600';
    return 'text-accent-500';
}

function focusGradientColor(float $score): string {
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
    ...
    <div class="bg-white rounded-xl border border-gray-100 p-5 text-center">
        ...
        <p class="text-2xl font-bold <?= focusColor($userFocusScore) ?>"><?= $userFocusScore ?></p>
    ...
    <div class="bg-white rounded-xl border border-gray-100 p-6">
        ...
        <div class="flex justify-center mb-4">
            <div class="relative w-28 h-28">
                <?php
                $fsCol = focusGradientColor($userFocusScore);
                $arcLen = round(2 * M_PI * 45 * 270 / 360);
                $arcOff = round($arcLen * (1 - $userFocusScore / 100));
                ?>
                <svg viewBox="0 0 120 120" class="w-28 h-28">
                    <path d="M 60 105 A 45 45 0 1 1 60.001 105" fill="none" stroke="#e5e7eb" stroke-width="8" stroke-linecap="round" transform="rotate(135 60 60)" stroke-dasharray="<?= $arcLen ?>" stroke-dashoffset="0"/>
                    <path d="M 60 105 A 45 45 0 1 1 60.001 105" fill="none" stroke="<?= $fsCol ?>" stroke-width="8" stroke-linecap="round" transform="rotate(135 60 60)" stroke-dasharray="<?= $arcLen ?>" stroke-dashoffset="<?= $arcOff ?>"/>
                </svg>
                <div class="absolute inset-0 flex flex-col items-center justify-center">
                    <span class="text-2xl font-bold" style="color:<?= $fsCol ?>"><?= $userFocusScore ?></span>
                    <span class="text-xs text-muted">de 100</span>
                </div>
            </div>
        </div>

        <?php if ($userFocusBreakdown): ?>
        <div class="space-y-2 text-sm">
            <div class="flex justify-between">
                <span class="text-muted">Productividad</span>
                <span class="font-medium text-dark"><?= $userFocusBreakdown['productivity'] ?>%</span>
            </div>
            <div class="flex justify-between">
                <span class="text-muted">Constancia</span>
                <span class="font-medium text-dark"><?= $userFocusBreakdown['constancy'] ?>%</span>
            </div>
            <div class="flex justify-between">
                <span class="text-muted">Deep Work</span>
                <span class="font-medium text-dark"><?= round($userFocusBreakdown['deep_work'] / 60) ?>m</span>
            </div>
            <div class="flex justify-between">
                <span class="text-muted">Cambios de App</span>
                <span class="font-medium text-dark"><?= $userFocusBreakdown['switches'] ?></span>
            </div>
            <div class="flex justify-between">
                <span class="text-muted">Puntualidad</span>
                <span class="font-medium text-dark"><?= $userFocusBreakdown['punctuality'] >= 0 ? '+' : '' ?><?= $userFocusBreakdown['punctuality'] ?>m</span>
            </div>
        </div>
        <?php else: ?>
        <div class="space-y-2 text-sm">
...
<?php if (!empty($userDualAlerts)): ?>
<!-- Dual-Job Alert Banner -->
<div class="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-4 sm:mb-6">
    <div class="flex items-center gap-3 mb-2">
        ...
        <div>
            <p class="text-sm font-semibold text-amber-800"><?= count($userDualAlerts) ?> alerta<?= count($userDualAlerts) !== 1 ? 's' : '' ?> de actividades</p>
            <p class="text-xs text-amber-600">Detecciones recientes para este usuario</p>
        </div>
    </div>
    <div class="space-y-1.5">
        <?php
        $djLabels = ['after_hours_pattern' => 'Fuera de Horario', 'foreign_app' => 'App Sospechosa', 'remote_desktop' => 'Escritorio Remoto', 'suspicious_idle' => 'Inactividad Sospechosa'];
        foreach ($userDualAlerts as $dja): ?>
        <div class="flex items-center justify-between text-xs">
            <span class="text-amber-700"><?= $djLabels[$dja['alert_type']] ?? $dja['alert_type'] ?> — <?= date('d/m/Y', strtotime($dja['day_date'] ?? $dja['created_at'])) ?></span>
            <?php if ($dja['is_reviewed']): ?>
                <?php if (($dja['review_result'] ?? '') === 'unproductive'): ?>
                    <span class="text-red-600 font-medium">Improductivo</span>
                <?php elseif (($dja['review_result'] ?? '') === 'productive'): ?>
                    <span class="text-emerald-600 font-medium">Productivo</span>
                <?php else: ?>
                    <span class="text-emerald-600 font-medium">Revisada</span>
                <?php endif; ?>
            <?php else: ?>
                <span class="text-amber-800 font-medium">Pendiente</span>
            <?php endif; ?>
        </div>
        <?php endforeach; ?>
    </div>
