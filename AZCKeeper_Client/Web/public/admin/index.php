...
try {
    $stFocus = $pdo->prepare("
        SELECT ROUND(AVG(f.focus_score), 0) AS avg_fs
        FROM keeper_focus_daily f
        INNER JOIN keeper_users u ON u.id = f.user_id AND u.status = 'active'
        LEFT JOIN keeper_user_assignments ua ON ua.keeper_user_id = u.id
        WHERE f.day_date BETWEEN :dfrom AND :dto
        {$scope['sql']}
    ");
    $stFocus->execute(array_merge($params, [':dfrom' => $dateFrom, ':dto' => $dateTo]));
    $avgFocusScore = (int)$stFocus->fetchColumn();
} catch (\Throwable $e) {
    ...
    <div class="bg-white rounded-xl border border-gray-100 p-4 sm:p-6">
        ...
        <div class="flex justify-center mb-3">
            <div class="relative w-32 h-32">
                <?php
                $fsColor = $avgFocusScore >= 75 ? '#10b981' : ($avgFocusScore >= 50 ? '#f59e0b' : '#be1622');
                $arcTotal = 2 * M_PI * 45 * 270 / 360;
                $arcOffset = round($arcTotal * (1 - $avgFocusScore / 100));
                ?>
                <svg viewBox="0 0 120 120" class="w-32 h-32">
                    <path d="M 60 105 A 45 45 0 1 1 60.001 105" fill="none" stroke="#e5e7eb" stroke-width="8" stroke-linecap="round" transform="rotate(135 60 60)" stroke-dasharray="<?= round($arcTotal) ?>" stroke-dashoffset="0"/>
                    <path d="M 60 105 A 45 45 0 1 1 60.001 105" fill="none" stroke="<?= $fsColor ?>" stroke-width="8" stroke-linecap="round" transform="rotate(135 60 60)" stroke-dasharray="<?= round($arcTotal) ?>" stroke-dashoffset="<?= $arcOffset ?>"/>
                </svg>
                <div class="absolute inset-0 flex flex-col items-center justify-center">
                    <span class="text-3xl font-bold" style="color:<?= $fsColor ?>"><?= $avgFocusScore ?></span>
        ...
        <p class="text-xs text-muted mt-3 leading-relaxed">Keeper analiza patrones de interacción y metadatos de ventanas para evaluar la dedicación del servicio.</p>
        <?php if (canAccessModule('productivity')): ?>
        <a href="productivity.php" class="inline-flex items-center gap-1 text-xs text-corp-800 hover:text-corp-600 font-medium mt-2 transition-colors">
            Ver detalle <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
        </a>
        <?php endif; ?>
    </div>
...
<?php if ($dualJobPending > 0 && canAccessModule('dual_job')): ?>
<!-- Dual-Job Alert Banner -->
<div class="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-4 sm:mb-8">
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
            ...
            <div>
                <p class="text-sm font-semibold text-amber-800"><?= $dualJobPending ?> alerta<?= $dualJobPending !== 1 ? 's' : '' ?> de actividades pendiente<?= $dualJobPending !== 1 ? 's' : '' ?></p>
                <p class="text-xs text-amber-600">Detecciones automáticas que requieren revisión</p>
            </div>
        </div>
        <a href="dual-job-alerts.php?status=pending" class="px-4 py-2 bg-amber-600 text-white text-xs font-semibold rounded-lg hover:bg-amber-700 transition-colors flex-shrink-0">
            Revisar
        </a>
    </div>
</div>
